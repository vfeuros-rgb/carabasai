...
export async function POST(request: Request) {
  ...
  try {
    ...
    const screenplay = await complete(writer, `Produce the final shooting-ready screenplay. Apply useful director and dialogue-editor notes while protecting the brief. Enforce the voice bible and knowledge ledger. No character may know information they have not acquired, explain a feeling that can be performed, repeat shared information, or speak without an immediate intention. Read every exchange as a chain of reactions. Output only the screenplay.\n\n${STRUCTURE}\n\n${brief}\n\nVOICE BIBLE:\n${voiceBible}\n\nDIALOGUE LOGIC PLAN + KNOWLEDGE LEDGER:\n${dialoguePlan}\n\nDRAFT:\n${draft}\n\nDIALOGUE EDITOR NOTES:\n${dialogueNotes}\n\nDIRECTOR NOTES:\n${directorNotes}`, controller.signal);
    const dialogueAudit = await complete(writer, `Audit the final screenplay without rewriting it. Score 0-10: causal logic, response coherence, distinct voices, subtext, playability, everyday credibility, power/information change and knowledge continuity. Also score exposition overload and AI-cringe from 0 (none) to 10 (severe). For every score below 7, cite the exact short fragment and state one concrete correction. Check that every spoken line has an action verb and that no character knows information without a traceable source. Return a compact audit.\n\nVOICE BIBLE:\n${voiceBible}\n\nKNOWLEDGE LEDGER:\n${dialoguePlan}\n\nFINAL SCREENPLAY:\n${screenplay}`, controller.signal);
    return NextResponse.json({ screenplay, director_notes: directorNotes, dialogue_notes: dialogueNotes, dialogue_audit: dialogueAudit, dialogue_plan: dialoguePlan, voice_bible: voiceBible, reference_count: referenceGroups.flat().length, dialogue_reference_count: dialogueReferences.length, screenwriter_profile: writer.id, director_profile: director.id });
  } catch (error) {
