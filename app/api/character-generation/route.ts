...
async function waitForPrediction(token: string, prediction: ReplicatePrediction) {
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    ...
    if (!response.ok) throw new Error(`Replicate status request failed with ${response.status}`);
...
export async function POST(request: Request) {
  ...
  if (!token) {
    return NextResponse.json({ error: "CHARACTER GENERATION IS NOT CONFIGURED." }, { status: 503 });
  }
  ...
  const specialist = characterCastingSpecialists.find((item) => item.id === body.specialistId);

  if (!projectId || !/^[0-9a-f-]{36}$/i.test(projectId)) {
    return NextResponse.json({ error: "VALID PROJECT ID IS REQUIRED." }, { status: 400 });
  }
  if (!specialist) {
    return NextResponse.json({ error: "CHARACTER CASTING SPECIALIST IS REQUIRED." }, { status: 400 });
  }
  if (!characterBrief) {
    return NextResponse.json({ error: "CHARACTER BRIEF IS REQUIRED." }, { status: 400 });
  }

  const model = process.env[specialist.generation.modelEnvironmentVariable]?.trim();
  if (!model || !/^[a-z0-9_.-]+\/[a-z0-9_.-]+(?::[a-f0-9]{64})?$/i.test(model)) {
    return NextResponse.json({ error: "THE SELECTED CASTING STYLE IS NOT CONNECTED." }, { status: 503 });
  }
  ...
  const replicateResponse = await fetch(
    ...
    body: JSON.stringify({
      ...
      input: {
        ...
        aspect_ratio: body.aspectRatio ?? specialist.generation.defaultAspectRatio,
  ...
  if (!replicateResponse.ok) {
    console.error("Replicate character generation failed", replicateResponse.status, await replicateResponse.text());
    return NextResponse.json({ error: "CHARACTER COULD NOT BE GENERATED." }, { status: 502 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "CHARACTER GENERATION TIMED OUT." }, { status: 504 });
  }

  const outputUrl = firstOutputUrl(prediction.output);
  if (prediction.status !== "succeeded" || !outputUrl) {
    console.error("Replicate character generation ended without output", prediction.error ?? prediction.status);
    return NextResponse.json({ error: "CHARACTER GENERATION FAILED." }, { status: 502 });
  }

  const imageResponse = await fetch(outputUrl, { cache: "no-store" });
  if (!imageResponse.ok) {
    return NextResponse.json({ error: "GENERATED CHARACTER COULD NOT BE DOWNLOADED." }, { status: 502 });
  }
  ...
  if (uploadError) {
    ...
    return NextResponse.json({ error: "GENERATED CHARACTER COULD NOT BE SAVED." }, { status: 502 });
  }
