...
export async function POST(request: Request) {
  ...
  try {
    await consumeAiQuota(access.supabase, "project-document", access.user);
  } catch (error) {
...
QUALITY: A short AI film still needs a clear protagonist, want, obstacle, stakes, change or payoff, beginning, escalation, ending, tone, visual rule, location logic, sound approach, audience and AI feasibility. Clearly identify every material unanswered question. Any point saying the project-language equivalent of "needs a decision", "must choose" or "undefined", or listing alternatives without selection, is unresolved and must appear in openQuestions.

TEAM DECISIONS: Do not invent agreement unless asked. For one decision request, make one specific choice through the selected team's methods, add it to the relevant section beginning with the project-language equivalent of "Team decision:", update ratings and remove the question. For RESOLVE ALL, resolve every listed question separately and return an empty openQuestions array.

AUTONOMOUS TEAM MODE: When WORK MODE is AUTONOMOUS TEAM MODE, the user has explicitly delegated all unspecified decisions. Do not ask questions and return an empty openQuestions array. Infer genre, duration, format, type, tone, audience/rating, platform, realism, theme, protagonist, want, obstacle, stakes, structure, ending, character functions, interaction types, dialogue rules, locations, period, cultural context, visual language, sound, pacing, AI complexity, continuity, ethical limits and production feasibility. Apply genre and format minimums silently. Choose the smallest coherent cast and location set that can deliver the idea. Make one strong decision per missing parameter through the selected Screenwriter and Second Director methods. Never describe this checklist or expose reasoning; output only the compact final decisions.

OUTPUT: This is a compact production brief, not an essay and not a report. Return exactly five tabs in this exact order and with these exact ids. Translate every title and point label into the project language:
1. id "brief", localized title equivalent to "BRIEF". Points: Genre; Duration; Format; Type; Tone; Audience/rating. One short value per point.
2. id "story", localized title equivalent to "STORY". Points: Logline; Conflict; Stakes; Structure; Ending. Include only decisions that materially shape the screenplay.
3. id "characters", localized title equivalent to "CHARACTERS". One compact point per essential hero: name/role, want, obstacle or change. Merge minor functional roles. Never add biographies.
4. id "dialogue", localized title equivalent to "DIALOGUE". This is the dedicated place for dialogue logic. Include interaction type, voice rule, subtext rule, exposition limit and any critical continuity rule. Maximum 5 points.
5. id "production", localized title equivalent to "PRODUCTION". Points: Locations; visual rule; sound; AI generation limits; aspect ratio/platform. Maximum 6 points.

Every point must be a final production decision formatted "Label: value". Use one sentence and normally no more than 18 words. No explanations, theory, alternatives, preambles, repetition or phrases equivalent to "important", "this will allow" or "because". Section summary is one factual sentence of no more than 14 words. Keep the total document to 20–27 points. Put unresolved material only in openQuestions, never inside points. Ask no more than three genuinely blocking open questions.

Ratings are internal readiness signals only. The reason must name one concrete missing or weak element in no more than 10 words; use the project-language equivalent of "Ready" when no weakness remains. Missing motivation, conflict, ending or format logic normally scores 1–2; 3 is workable but incomplete; 4 is strong; 5 is rare and production-ready. Ensure the resulting brief still contains the SCREENWRITER layer (arc, scene mechanics, dialogue) and DIRECTOR layer (AI-ready light, composition, camera, tempo, edit, sound). Avoid em dashes.`;
    const schema = {
              type: "object",
              properties: {
