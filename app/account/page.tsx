  ...
  if (userEmail) return <main className="min-h-screen bg-[#050505] text-white">
    ...
    <section className="min-h-screen pt-16 md:pl-[var(--studio-sidebar-width,260px)] md:pt-0">
      <div className="mx-auto w-full max-w-[1500px] px-4 py-7 sm:px-8 sm:py-10 lg:px-14">
        <header className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-[10px] font-black tracking-[0.18em] text-[#FFDF00]">{name || "CREATOR"}</p><h1 className="mt-3 text-3xl font-black tracking-[-0.05em] sm:text-4xl">WELCOME BACK TO YOUR STUDIO.</h1><p className="mt-3 text-sm text-white/35">{accountSessions.length} active {accountSessions.length === 1 ? "project" : "projects"} in your workspace.</p></div><div className="flex gap-2"><Link href="/account/data" className="bg-[#111] px-5 py-3 text-[9px] font-black text-white/50 hover:text-[#FFDF00]">ACCOUNT DATA</Link><button type="button" onClick={() => { setTrashedProjects(getTrashedProjects<AccountSession>()); setTrashOpen(true); }} className="bg-[#111] px-5 py-3 text-[9px] font-black text-white/50 hover:text-red-300">TRASH · {trashedProjects.length}</button></div></header>
